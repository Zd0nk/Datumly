...
# ══════════════════════════════════════════════════════════════════════
#  8.  POSITION-SPECIFIC SCORING (v5.2 corrected weights)
# ══════════════════════════════════════════════════════════════════════
...
def score_def(df):
    ...
    # v5.2: CS=4pts (biggest DEF earner), DC=2pts (half a CS), Goals=6pts
    w = {
        "ep_next":          0.18,
        "cs_prob_col":      0.30,   # CS=4pts — biggest single DEF earner
        "dc_hit_rate_col":  0.15,   # DC=2pts — half a CS
        "att_ret_p90":      0.18,   # Goals(6pts) + assists(3pts)
        "fix_s":            0.12,
        "xg_p90":           0.11,   # Goals alone (6pts each)
        "yellow_rate":     -0.06,   # -1pt per yellow
    }
...
def score_mid(df):
    ...
    # v5.2: CS=1pt only (near irrelevant for MID), Goals=5pts, DC=2pts > CS
    w = {
        "ep_next":          0.20,
        "xg_p90":           0.22,   # Goals=5pts — primary MID earner
        "xa_p90":           0.16,   # Assists=3pts
        "kp_p90":           0.10,
        "shots_p90":        0.09,
        "dc_hit_rate_col":  0.10,   # DC=2pts — more valuable than CS for MID
        "fix_s":            0.08,
        "cs_prob_col":      0.02,   # CS=only 1pt for MID
        "bonus_p90":        0.05,
        "yellow_rate":     -0.06,
    }
...
def score_fwd(df):
    ...
    w = {
        "ep_next":   0.28, "xg_p90":   0.22, "npxg_p90": 0.15,
        "shots_p90": 0.13, "xa_p90":   0.10, "fix_s":    0.08,
        "bonus_p90": 0.04,
    }
...
def apply_position_scores(df):
    parts = [score_gkp(df), score_def(df), score_mid(df), score_fwd(df)]
    ...
    for _ in range(num_transfers):
        ...
        if not in_row.empty:
            ...
            updated_squad = pd.concat(
                [updated_squad, in_row.iloc[[0]]], ignore_index=True
            )
            updated_squad_ids.discard(out_fpl_id)
            updated_squad_ids.add(int(in_row.iloc[0]["fpl_id"]))
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        st.subheader("v5.2 Scoring Reference")
        st.markdown("""
        ...
        **Captain projection** uses `cs × 4.0` for DEF/GKP (fixed from `× 6.0` in v5.1).
        """)
    ...
    # ── Attach actual sell prices from the picks endpoint ───
    ...
    # The picks endpoint provides the actual selling_price for each player.
    sell_price_map = {}
    for _, pk in picks.iterrows():
        pid = int(pk["element"])
        sp  = pk.get("selling_price", None)
        if sp is not None:
            sell_price_map[pid] = float(sp) / 10  # pence → £m
    # If selling_price not available (older API), fall back to now_cost
    if sell_price_map:
    ...
    show_cols = ["web_name","team_short","position","price","sell_price","form","ep_next",
                 "cs_prob","xg_p90","xa_p90","dc_hit_rate","avg_fix_diff",
